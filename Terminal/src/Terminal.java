...
public class Terminal {
	 ...
	 * @throws Exception
	 */
	public Terminal(String hostName, int hostPort, byte[] staticKeyData) throws Exception {
	...
	void initializeSession() throws Exception {
		...
		if (apdu.getStatus() == SW_NO_ERROR) {
			...
			// challenge
			byte[] keyDerivationData = generateKeyDerivationData(hostChallenge, cardChallenge);
	 ...
	 * @throws Exception
	 */
	protected byte[] processRequest(byte type, byte[] requestMessage) throws Exception {
	 ...
	 */
	protected byte[] generateKeyDerivationData(byte[] hostChallenge, byte[] cardChallenge) {
		...
		System.arraycopy(hostChallenge, 0, keyDerivationData, 0, CHALLENGE_LENGTH);
		// Append card challenge to derivation data
		System.arraycopy(cardChallenge, (short) 0, keyDerivationData, CHALLENGE_LENGTH, CHALLENGE_LENGTH);
	 ...
	 *
	 */
	protected void generateSessionKey(byte[] keyDerivationData) throws Exception {
	...
	private class Mac {
		...
		public Mac(SecretKey key) throws Exception {
			cipher = Cipher.getInstance("DES/CBC/NoPadding");
		...
		/**
		 * Generates a message MAC: generates the MAC and appends the MAC to the
		 * message.
		 *
		 * @param buffer
		 *            The APDU buffer
		 ...
		 * @throws Exception
		 */
		protected short generateMAC(byte[] buffer, int offset) throws Exception {
